...
(function () {
  ...
  function initC(root) {
    ...
    function controls(animate) {
      ...
      // stagger each element in so the option block cascades in rather than snapping as one chunk
      var anim = function (d) { return animate ? 'animation:scRise .32s ease both;animation-delay:' + d + 's;' : ''; };
      var zipOk = zip5(st.zip);
      var h = '<div style="align-self:stretch;margin-top:3px;">';
      if (st.stage > 0) h += '<button type="button" data-act="back" style="' + anim(0) + 'cursor:pointer;border:none;background:none;color:#8a857c;font:800 13px Mulish;padding:0 0 9px;display:inline-flex;align-items:center;gap:5px;">← Back</button>';
      if (st.stage === 0)
        h += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:9px;">' + PROBLEMS.map(function (p, i) {
          var active = st.problem === p.id;
          return '<button type="button" data-pick="problem" data-id="' + p.id + '" style="' + anim(0.05 + i * 0.05) + 'position:relative;text-align:left;cursor:pointer;background:#fff;border:1.5px solid ' + (active ? RED : '#e6e1d8') + ';border-radius:13px;padding:13px 14px;font-family:Mulish,sans-serif;display:flex;flex-direction:column;gap:3px;">' + (active ? '<span style="position:absolute;inset:0;border:2px solid ' + RED + ';border-radius:13px;background:rgba(206,63,38,.05);pointer-events:none;"></span>' : '') + '<span style="display:flex;align-items:center;gap:8px;"><span style="width:10px;height:10px;border-radius:50%;background:' + p.accent + ';flex:none;"></span><span style="font:800 14.5px Mulish;color:#1b1a18;">' + esc(p.label) + '</span></span><span style="font:500 12px Mulish;color:#8a857c;padding-left:18px;">' + esc(p.sub) + '</span></button>';
        }).join('') + '</div>';
      if (st.stage === 1)
        h += '<div style="display:flex;flex-direction:column;gap:9px;">' + URGENCIES.map(function (u, i) {
          var active = st.urgency === u.id;
          return '<button type="button" data-pick="urgency" data-id="' + u.id + '" style="' + anim(0.05 + i * 0.055) + 'position:relative;text-align:left;cursor:pointer;background:#fff;border:1.5px solid ' + (active ? RED : '#e6e1d8') + ';border-radius:12px;padding:13px 15px;font-family:Mulish,sans-serif;display:flex;align-items:center;justify-content:space-between;gap:10px;">' + (active ? '<span style="position:absolute;inset:0;border:2px solid ' + RED + ';border-radius:12px;background:rgba(206,63,38,.05);pointer-events:none;"></span>' : '') + '<span style="display:flex;flex-direction:column;gap:1px;"><span style="font:800 15px Mulish;color:#1b1a18;">' + esc(u.label) + '</span><span style="font:500 12px Mulish;color:#8a857c;">' + esc(u.sub) + '</span></span><span style="font:800 11px Mulish;color:' + u.accent + ';background:' + u.tagBg + ';padding:5px 10px;border-radius:99px;white-space:nowrap;">' + esc(u.tag) + '</span></button>';
        }).join('') + '</div>';
      if (st.stage === 2) {
        h += '<div style="' + anim(0.04) + 'position:relative;margin-bottom:9px;"><input data-in="zip" inputmode="numeric" maxlength="5" aria-label="ZIP code" value="' + esc(st.zip) + '" placeholder="ZIP code" style="width:100%;border:1.5px solid #e2ddd5;border-radius:11px;padding:14px 44px 14px 15px;font:600 16px Mulish;background:#fff;outline:none;">' + (zipOk ? '<span style="position:absolute;right:13px;top:50%;transform:translateY(-50%);width:24px;height:24px;border-radius:50%;background:#4ec97a;color:#fff;display:flex;align-items:center;justify-content:center;font:900 13px Mulish;animation:scPop .3s ease;">✓</span>' : '') + '</div>';
